...
#[cfg(not(feature = "gl"))]
use winit::{Event, Window as BackWindow, WindowBuilder, WindowEvent};
use winit::EventsLoop;

#[cfg(feature = "gl")]
use crate::gfx_back::{Backend, config_context, glutin::*, Surface};
...
impl Window {
	...
	pub fn new(dims: (u32, u32)) -> Self {
		...
		let window = Self::make_builder(dims)
	...
	pub fn new(dims: (u32, u32)) -> Self {
		...
		let window = {
			let builder =
				config_context(ContextBuilder::new(), Format::Rgba8Srgb, None)
					.with_vsync(true);
			GlWindow::new(wb, builder, &events_loop).unwrap()
		};
		let surface = Some(Surface::from_window(window));
		Window { events_loop, dims, surface }
	}
