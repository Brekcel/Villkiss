...
use gfx_hal::{adapter::Adapter, Graphics, Device,
	Instance, QueueFamily, QueueGroup, Submission, Swapchain as HAL_Swapchain, Surface};
#[cfg(not(feature = "gl"))]
use gfx_hal::adapter::DeviceType;

use crate::{Fence, Semaphore, Swapchain, Window};
#[cfg(not(feature = "gl"))]
use crate::gfx_back;
...
impl HALData {
	...
	pub fn present(&self, swap: &Swapchain, frame_idx: u32, present_sems: Vec<&Semaphore>) -> Result<(), ()> {
		let queue = &mut self.queue_group.borrow_mut().queues[0];
		let swap = unsafe { swap.swapchain.get_ref() };
		let present_sems = present_sems.iter().map(|s| s.semaphore()).collect::<Vec<_>>();
